...
while x != root[T] and color[x] = BLACK                              // x 始终指向具有双重黑色的非根结点
 ...
    if color[w]= RED                                                 // CASE5: x 为 BLACK, w 为 RED, p[x] 为 BLACK. 转化为2/3/4.
      color[w] <- BLACK                                               
      color[p[x]] <- RED                                            
      RIGHT-ROTATE(T,p[x])                                             
      w <- left[p[x]]                                               // w 原来是 RED,所有其两个孩子必然是 BLACK, 所以现在的 w 为 BLACK
    if color[left[w]] = BLACK and color[right[w]] = BLACK           // CASE6: x,w 为 BLACK, 且 w 两个孩子都为 BLACK
      color[w] <- RED                                               // 将p[x] 左子树黑高减少1
      x <- p[x]                                                     // p[x]的左右子树黑高都少1,因此需求的额外 BLACK 上移一层
    else if color[left[w]] = BLACK                                  // CASE7: x,w 为 BLACK, 且 w 左孩子 BLACK, 右孩子 RED. 转化为4
           color[right[w]] <- BLACK
           color[w] <- RED
           LEFT-ROTATE(T,w)
           w <- left[p[x]]
         color[w] <- color[p[x]]                                    // CASE8: x,w 为 BLACK, 且 w 左孩子 RED
         color[p[x]] <- BLACK                                       // 右旋后右子树增加一个黑结点,黑高加1
         color[left[w]] <- BLACK                                    // 右旋后保证左子树黑高不变
         RIGHT-ROTATE(T,p[x]])                                      // 右旋为 右子树 增加一个结点, 即为原来的 p[x]
         x <- root[T]                                               // 到这一步,已完成调整,可以结束循环
color[x] <- BLACK  
